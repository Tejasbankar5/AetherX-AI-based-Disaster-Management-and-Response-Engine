...
const MapController = ({ view }: { view: { lat: number; lng: number; zoom: number } | null }) => {
    ...
    useEffect(() => {
        if (view) {
            // Only flyTo if the TARGET view has changed significantly from the LAST APPLIED view.
            // This prevents the map from snapping back when the parent re-renders but the view prop hasn't logically changed.
            const isSame = lastView &&
                Math.abs(lastView.lat - view.lat) < 0.0001 &&
                Math.abs(lastView.lng - view.lng) < 0.0001 &&
    ...
    }, [view, map, lastView, setLastView]);

    useEffect(() => {
        const handleVoice = (e: any) => {
            if (e.detail?.type === 'FOCUS') {
                const city = e.detail.payload.toLowerCase();
                // Hacky geocoding for demo
                ...
                if (lookup[city]) {
                    map.flyTo(lookup[city], 12, { duration: 2 });
                }
            }
        };
