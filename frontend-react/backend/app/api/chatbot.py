...
def chatbot_reply(request: ChatRequest):
    ...
    try:
        ...
        if "evacuate" in msg or "safe" in msg:
            reply = translate('fallback_evacuate', lang)
        elif "flood" in msg:
            reply = translate('fallback_flood', lang)
        elif "earthquake" in msg:
            reply = translate('fallback_earthquake', lang)
        elif "cyclone" in msg or "storm" in msg:
            reply = translate('fallback_cyclone', lang)
        elif "help" in msg or "emergency" in msg:
            reply = translate('fallback_help', lang)
        elif any(x in msg for x in ["hello", "hi", "hey", "greetings", "namaste", "नमस्कार"]):
            reply = translate('fallback_greeting', lang)
        else:
            reply = translate('fallback_default', lang)
